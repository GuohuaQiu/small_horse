...
{
	TRACE("OnMatchString; %s  <>   %s\n", s, sMatch);
	return (-1 != sMatch.Find(s));
...
LRESULT CAutoComplete::WindowProc(UINT msg, WPARAM wp, LPARAM lp)
{
   if ((msg==WM_COMMAND && LOWORD(wp)==m_idMyControl) &&
      ...
      TRACE("m_bIgnoreChangeMsg = %d\n",m_bIgnoreChangeMsg);
      if (!m_bIgnoreChangeMsg++) {
      TRACE("m_bIgnoreChangeMsg = can work\n");
         CString s;
...
//	  else
//	  {
//		  pComboBox->SetWindowText( arMatches[0] );
//		  int pos = arMatches[0].Find(s);
//		  pComboBox->SetEditSel(pos+ s.GetLength(),-1 );
//	  }
//      pComboBox->SetWindowText(IgnoreCompletion(s) ? s : arMatches[0]);


#if 0   // from home ,no test which one is good.
	  ...
	  else
	  {
		  pComboBox->SetWindowText( s );
		  int pos = arMatches[0].Find(s);
		  pComboBox->SetEditSel(pos,pos + s.GetLength());
